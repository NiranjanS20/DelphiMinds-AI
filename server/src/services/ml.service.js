...
const fallbackParseV2Payload = (reason = 'ML v2 service unavailable') => ({
  parser_metadata: {
    parser_version: 'v2',
...
const callParseResumeV2 = async ({ filePath, originalName, mimetype }) => {
  const endpoint = ML_BASE_URL + '/parse-resume-v2';

  if (!filePath || !fs.existsSync(filePath)) {
    logger.warn('ML v2 parse skipped because resume file is missing', { filePath });
    return fallbackParseV2Payload('Resume file is missing');
  }
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      ...
      const response = await axios.post(endpoint, formData, {
        headers: formData.getHeaders(),
        timeout: 60000, // Gemini processing needs more time than local parsing
        maxBodyLength: Infinity,
      });
      ...
      return {
        ...
        meta: {
          fallback: false,
          source: 'ml-service-v2',
        },
      };
    } catch (error) {
      attempt += 1;
      logger.warn('ML v2 parse attempt failed', {
        attempt,
        error: error.message,
      });

      if (attempt > MAX_RETRIES) {
        logger.error('ML v2 parse failed, using fallback response', {
          error: error.message,
        });
        return fallbackParseV2Payload(error.message);
      }
    }
  }

  return fallbackParseV2Payload();
};
