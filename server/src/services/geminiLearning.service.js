...
const callGeminiLearning = async ({ systemPrompt, message, history = [] }) => {
  if (!env.geminiLearningApiKey) {
    throw new AppError(
      'Gemini Learning API key is not configured',
      500,
      errorCodes.EXTERNAL_SERVICE_ERROR
    );
  }

  const endpoint = `${env.geminiApiUrl.replace(/\/$/, '')}/models/${env.geminiModel}:generateContent?key=${env.geminiLearningApiKey}`;
  const turns = history
    .slice(-8)
